...
Linux `perf` 首先按开销对条目进行排序，因此我们需要手动过滤我们感兴趣的分支的条目。幸运的是，它们可以通过 `grep` 命令轻松过滤。事实上，如果我们过滤它们，我们将得到以这个分支结尾的基本块的延迟分布，如表 {@tbl:bb_latency} 所示。这些数据可以绘制成一个类似于图 [@fig:LBR_timing_BB](#LBR_timing_BB) 的图表。
