...
代码清单:循环展开和阻塞 {#lst:unrolljam .cpp}
```cpp
for (int i = 0; i < N; i++)           for (int i = 0; i+1 < N; i+=2)
...
代码清单:不能将strlen移出循环 {#lst:Restrict .cpp}
```cpp
